...
4. **`output_power: 8.5`, `fast_connect`.** The rest of the documented combo.
   Nothing. (`output_power` *lowers* transmit power, which was precisely the
   wrong direction.)
...
did the thing I should have done first, turning on debug logging to read the
...
| --- | ---: | --- |
| Onboard antenna, `cocean` (across room) | **−95 dBm** | instant `Auth Expired` |
| Onboard antenna, an AP in the same room | −86 dBm | instant reject |
| Moved it closer | −66 dBm | *associates*, then stalls ~46 s |
| **External antenna** + close AP | −54 dBm | reaches the handshake |
| External antenna, `cocean`, correct pw | −88 dBm | connected |
...
The last row is the one that gives the game away: −88 dBm connected where −86 dBm
had been rejected outright. RSSI only measures what the board *hears*. The
external antenna's real work was on transmit, which no number in this table
shows.

The culprit was the **antenna**. The XIAO ESP32-S3 Sense has a tiny onboard trace
antenna, and on the *Sense* the camera daughterboard sits over the RF section,
which I assume is not helping. It ships with a little external U.FL antenna that you're meant to plug
...
- **`Auth Expired` ≠ wrong password.** A wrong password fails *later*, in the key
  exchange, after the device has associated. Failing at the auth stage with a
  *known-good* password points at the physical layer. (`4-Way Handshake Timeout`
  is the ambiguous one: it is where a bad PSK lands, but weak signal reaches it
  too, as the −66 dBm row above shows.)
...
The pod is up now, streaming frames to the dashboard over ordinary house Wi-Fi.
The last mile turned out to be the oldest lesson in radio: **check your
signal.**