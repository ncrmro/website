...
Not a whole platform. Not a dashboard.
...
Outfitter does not hide the agent CLI; it makes the setup reproducible.
...
Agent workflows should not require copy-pasting a bunch of prompt and CLI setup into every repo. They also should not require a separate agent identity per automation — see [Few Profiles, Many Skills](/posts/few-profiles-many-skills) for the longer argument.
...
My bet is that agentic engineering workflows need a package manager more than a command center.
