...
candidates are visibly competing. Starship's thirteenth flight test ran the
loop in public in July 2026: three countdowns against the same stacked
vehicle, each a complete proposal, and mutually exclusive — the first one to
fly closes the launch period. That is a release train with competing
...
The two closures are not the same event. Attempt 1 is rejected on the merits,
and by a machine: the count reached T−0 and the vehicle aborted itself during
ignition on LOX turbopump faults — CI failing the deploy step, not a reviewer
changing their mind. Attempt 2 was never unviable; it was closed by an
environmental gate, scrubbed for weather because the ascent imagery of the
heat shield needed clear sky. Attempt 3 merged, and merging is what ended the
launch period. A candidate stays open until something else lands, which is the
ordinary fate of a pull request and the ordinary fate of a launch window.
...
computed against — section 2.4's staleness rule, enforcing itself.
Section 3 develops the notation on maneuvers; PR numbers and shas are
illustrative — the dates, times, and closure reasons here are Flight 13's.

The direction of causality is worth fixing before going further, because it is
the opposite of how software teams usually meet a repo. The repository does not
...
approved for merge. Every phase of the mission is one turn of that loop:
reality lands as a commit, the commit opens the branch for the next event,
the merge authorizes it, and the vehicle's response lands as the next commit.
